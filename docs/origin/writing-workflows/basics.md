...
::: warning `type: graph` is required here
The default is `type: chain`, which adds an implicit dependency on the preceding step
**on top of** whatever `depends` you write. Without the `type: graph` line below,
`task1` and `task2` would still run one after the other. See
[Execution Type](/reference/yaml#execution-type).
:::
