...
`type` controls how step dependencies are resolved:

- **`chain`** (default) — each step implicitly depends on the step defined before it,
  **in addition to** any `depends` you write yourself. Steps run one after another
  unless you opt out.
- **`graph`** — only explicit `depends` are honored. Steps with no dependency
  relationship run in parallel.
...
steps:
  ...
  - name: b
    command: ./b.sh
    depends: [a]     # written explicitly, but chain would add it anyway
  - name: c
    command: ./c.sh
    depends: [a]     # chain ALSO adds an implicit depends on b, so c waits for b
```

The same workflow under `type: graph` runs `b` and `c` in parallel, because only the
explicit `depends: [a]` counts.

To keep `chain` but let one step start without waiting for the previous one, set
`depends: []` explicitly — that opts the step out of implicit dependency injection.

Alternatively, a nested list declares a parallel group inside a chain:
...
steps:
  - name: prepare
    command: ./prepare.sh
  - - name: task-a       # task-a and task-b both depend on prepare,
      command: ./a.sh    # and run in parallel with each other
    - name: task-b
      command: ./b.sh
  - name: finish
...
| Field | Type | Description | Default |
|-------|------|-------------|---------|
| `maxActiveRuns` | integer | Max concurrent workflow runs (negative = queueing disabled) | `1` |
| `maxActiveSteps` | integer | Max parallel steps (`0` = unlimited) | `0` (unlimited) |
...
::: warning Deprecated
The DAG-level `ssh:` block and `executor.type: ssh` are deprecated. Both still work but
log a build warning. Use the `ssh2` executor instead — see
[SSH Executor](/features/executors/ssh).
:::

`ssh2` takes only a `host`, and reads everything else (user, port, key, jump hosts)
from your `~/.ssh/config`. It does **not** inherit the DAG-level `ssh:` block:
...
steps:
  - name: restart app
    executor:
      type: ssh2
      config:
        host: app.example.com     # or an alias from ~/.ssh/config
    command: systemctl restart myapp
```

**Note:** `ssh` and `container` fields are mutually exclusive at the DAG level.
