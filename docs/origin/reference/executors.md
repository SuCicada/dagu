...
Execute commands on a remote host by shelling out to the system `ssh` client.
...
steps:
  - name: remote task
    executor:
      type: ssh2
      config:
        host: server        # an entry in your ~/.ssh/config, or user@host
    command: /path/to/script.sh
```

`host` is passed straight to `ssh`, so all your `~/.ssh/config` settings (user, port,
identity file, jump hosts, agent forwarding) apply as usual. There is no separate
key/port/user configuration.

### Working Directory

A remote working directory can be set per step with `dir`, or for the whole DAG with
`workingDir`. The step-level `dir` wins. It is turned into a `cd <dir> && ...` prefix
on the remote command:

```yaml
workingDir: /home/peng/app     # applies to every ssh2 step
...
steps:
  - name: build
    executor:
      ...
      config:
        host: server
    dir: /home/peng/other      # overrides workingDir for this step
    command: make build
```

Note that a DAG-level `workingDir` that was merely *defaulted* (from the DAG file's own
location) is never sent to the remote host — only an explicitly set one is.

### Stopping a remote step

Aborting a DAG run kills the **local** `ssh` process, but the command on the remote host
keeps running: without a PTY, closing the connection does not deliver a signal to the
remote process.

If you need the remote side to die too, request a PTY yourself:

```yaml
    command: ssh -tt server 'long-running-thing'
```

or make the remote command clean up after itself (`trap`, `timeout`, a lock file).
This matters most for steps holding a lock — an aborted `restic`/`rclone` backup can
leave its repository locked.

::: warning Legacy `ssh` executor
`executor.type: ssh` is deprecated and logs a warning at runtime. Use `ssh2`.
:::
